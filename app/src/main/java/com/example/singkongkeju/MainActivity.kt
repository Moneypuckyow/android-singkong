package com.example.singkongkeju
...
import com.example.singkongkeju.databinding.ActivityMainBinding
import com.example.singkongkeju.ml.Model
...
class MainActivity : AppCompatActivity() {
    ...
    private fun outputGenerator(bitmap: Bitmap){
        ...
        val cassavaModel = Model.newInstance(this)
        ...
        val outputs = cassavaModel.process(tfimage)
    ...
    private val requestPermissionLauncher = registerForActivityResult(ActivityResultContracts.RequestPermission()){
            isGranted: Boolean ->
        if (isGranted){
            AlertDialog.Builder(this).setTitle("Download Gambar?")
                .setMessage("Apakah anda ingin mendownload gambar ini ke device anda?")
    ...
    private fun downloadImage(mBitmap: Bitmap):Uri? {
        val contentValues = ContentValues().apply {
            put(MediaStore.Images.Media.DISPLAY_NAME,"Cassava_Images"+ System.currentTimeMillis()/1000)
